...
class Transformer(nn.Module):
    ...
    def generate(
        ...
        # TODO: order optimized by RL?
        orders = torch.rand(256, device=device).argsort(dim=0) + 1  # random order for the complete prediction

        last_pos = 0  # linear idx. in orders list
        last_range = range(0, 1)  # initial for class token, hardcode
        sequences = []
        
        self.setup_kv_cache(enable=True)
        for step in range(num_iter):
            # TODO: mask_ratio optimized by RL?
            ...
            mask_len = max(1, min(seq_len - last_pos - 1, mask_len))  # remaining tokens to ignore at current step
            
            num_pred = seq_len - last_pos - mask_len  # remaining token to decode at current step
            if step == num_iter - 1:
                num_pred = seq_len - last_pos

            next_range = orders[range(last_pos, last_pos + num_pred)]  # current step random-ordered token pos. range
            last_pos += num_pred
